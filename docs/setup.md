...
```shell
cd workspace
git clone https://gitlab.com/Lipovsky/distsys-course.git
```
...
Если на предыдущем шаге вы развернули контейнер, то **залогинтесь в него и перейдите в корень репозитория** (т.е. директорию `distsys-course`). Если вы работаете без докера, то просто перейдите в корень репозитория.
...
Решите задачу [A+B](/tasks/all-tasks/0-aplusb).
