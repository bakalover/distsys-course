...
Интерфейс `ILog` не предполагает конкурентного доступа. Если вы собираетесь обращаться к нему из разных потоков / файберов, то должны самостоятельно продумать синхронизацию.

### Обрезание лога и снимки состояния
...
Чтобы ограничить расход места на диске, RSM должен периодически обрезать закоммиченный и примененный префикс лога и заменять его на _снимок состояния_ (_snapshot_).

Персистентное состояние RSM, таким образом, образовано
1) суффиксом лога команд и 
2) снимком состояния, отвечающего обрезанному префиксу.
...
Прокси эти ситуации не различает, он просто отправляет команду другой реплике.
...
- Любая отправленная в RSM команда применится к автомату только один раз.
...
Читающие операции можно линеаризовать относительно мутаций, и для этого необязательно проводить их через консенсус / фиксировать в логе.

Есть два базовых способа это делать:

### Quorum reads

Реплика, обслуживающая чтение, синхронно общается с кворумом (без записи в лог команд), выбирает слот для линеаризации чтения и локальное ждет коммита / применения мутации из этого слота.

### Leader leases

Если гарантировать, что лидеры не пересекаются во времени, то можно читать с лидера без коммуникации с другими узлами. 

С одной стороны, мы избавляемся от коммуникации с другими узлами при обслуживании чтения, но с другой ставим линеаризуемость системы в зависимость от поведения часов на отдельных репликах.
...
Для передачи команд / закоммиченных индексов между файберами используйте каналы.
